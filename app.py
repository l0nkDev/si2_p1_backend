...
def reportes(base, criteria, order, since, until, format): 
    ...
    if base == 'logs':
        query = "select id, username, role, email, action, ip, (datetime - interval '1 hour' * 4) as datetime from bitacora where {0} (datetime - interval '1 hour' * 4) >= '{1}' and (datetime - interval '1 hour' * 4) <= '{2}' order by id {3}".format('' if criteria == 'any' else f"role = '{criteria}' and ",since, until, order)
